...
<div class="jarviswidget " id="wid-id-4" data-widget-editbutton="false" data-widget-custombutton="false">
    ...
    <div>
        ...
        <form action="/${modelName?lower_case}/update" class="smart-form" method="post" id="${modelName?lower_case}Form">
        	<fieldset>
        		<div class="row">
				<#list table.columns as it>
					...
					<section class="col " style="width:33.33%">
						...
						<label class="input" >
							<input name="${modelName?lower_case}.${it.columnName}" placeholder="${it.comment}" type="text" value="\$\{${modelName?lower_case}.${it.columnName}\!\}">
						</label>
					</section>
					<#else>
					<input type="hidden" name="${modelName?lower_case}.${it.columnName}" value="\$\{${modelName?lower_case}.${it.columnName}\!\}"/>
					</#if>
					<#if it_index%3==0>
					</div>
					<div class="row">
			...
			<footer>
                ...
                <a onclick="if(modifyWinJO)modifyWinJO.remove()"><button type="button" class="btn btn-default " >关闭退出</button></a>
				<!--
...
<script type="text/javascript">
	...
	function mysubmit() {<#list table.columns as it><#if it.columnName!='ID'>
		...
		loadrightbydata("/${modelName?lower_case}/update", $('#${modelName?lower_case}Form').serializeObject());
